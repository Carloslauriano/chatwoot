module AccountSettingsSchema
  extend ActiveSupport::Concern

  CAPTAIN_MODEL_PROPERTIES = Llm::Models.model_feature_keys.index_with { { 'type': %w[string null] } }.freeze
  CAPTAIN_FEATURE_PROPERTIES = Llm::Models.feature_keys.index_with { { 'type': %w[boolean null] } }.freeze

  SETTINGS_PARAMS_SCHEMA = {
    'type': 'object',
    'properties':
      {
        'auto_resolve_after': { 'type': %w[integer null], 'minimum': 10, 'maximum': 1_439_856 },
        'auto_resolve_message': { 'type': %w[string null] },
        'auto_resolve_ignore_waiting': { 'type': %w[boolean null] },
        'audio_transcriptions': { 'type': %w[boolean null] },
        'auto_resolve_label': { 'type': %w[string null] },
        'keep_pending_on_bot_failure': { 'type': %w[boolean null] },
        'captain_auto_resolve_mode': { 'type': %w[string null], 'enum': ['evaluated', 'legacy', 'disabled', nil] },
        'captain_false_promise_harness_enabled': { 'type': %w[boolean null] },
        'conversation_required_attributes': {
          'type': %w[array null],
          'items': { 'type': 'string' }
        },
        'require_label_before_resolve': { 'type': %w[boolean null] },
        'require_priority_before_resolve': { 'type': %w[boolean null] },
        'require_assignee_before_resolve': { 'type': %w[boolean null] },
        'require_priority_before_assignment': { 'type': %w[boolean null] },
        'require_priority_before_reply': { 'type': %w[boolean null] },
        'require_team_fully_prioritized_before_assignment': { 'type': %w[boolean null] },
        'require_team_fully_prioritized_before_reply': { 'type': %w[boolean null] },
        'disable_snooze': { 'type': %w[boolean null] },
        'disable_pending': { 'type': %w[boolean null] },
        'auto_assign_on_agent_reply': { 'type': %w[boolean null] },
        'robot_sender_user_id': { 'type': %w[integer null] },
        'silent_resolve_enabled': { 'type': %w[boolean null] },
        'silent_resolve_label': { 'type': %w[string null] },
        'inactivity_resolve_enabled': { 'type': %w[boolean null] },
        'inactivity_resolve_label': { 'type': %w[string null] },
        'inactivity_resolve_message': { 'type': %w[string null] },
        'hidden_priorities': {
          'type': %w[array null],
          'items': { 'type': 'string', 'enum': %w[urgent high medium low] }
        },
        'captain_models': {
          'type': %w[object null],
          'properties': CAPTAIN_MODEL_PROPERTIES,
          'additionalProperties': false
        },
        'captain_features': {
          'type': %w[object null],
          'properties': CAPTAIN_FEATURE_PROPERTIES,
          'additionalProperties': false
        }
      },
    'required': [],
    'additionalProperties': true
  }.to_json.freeze
end
